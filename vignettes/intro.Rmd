...
And then compute some accuracy measures:

```{r accuracy}
fc_all %>%
  accuracy(deaths_test) %>%
  arrange(MAPE)
```


The GAM model can be analysed with the standard tools provided by `fable`, such as `glance()`, `tidy()`, `generate()`,`interpolate()` and more, in the same way as other model classes.
